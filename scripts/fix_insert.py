"""
Fix insert error and add remaining missing books.
Run on your Mac.
"""
import asyncio
import os
...
# Only the still-missing books
MISSING = [
    # EMOTIONS — critical, only 3
    ...
    ("Scaredy Squirrel", "Melanie Watt", ["emotions","courage"], (4,8)),
    ...
    # ENVIRONMENT — needs 1 more
    ("Watership Down", "Richard Adams", ["environment","resilience","friendship"], (11,14)),
    ("The Lorax", "Dr. Seuss", ["environment","courage"], (4,8)),
    ("Hoot", "Carl Hiaasen", ["environment","courage","problem_solving"], (9,13)),
    ("Flush", "Carl Hiaasen", ["environment","problem_solving"], (9,13)),
    ("Seedfolks", "Paul Fleischman", ["environment","community","kindness"], (8,12)),
    ...
    ("Click Clack Moo Cows That Type", "Doreen Cronin", ["problem_solving","creativity"], (3,7)),

    # EARLY READERS
    ("Frog and Toad Are Friends", "Arnold Lobel", ["friendship","kindness"], (4,8)),
    ("Junie B Jones and the Stupid Smelly Bus", "Barbara Park", ["friendship","problem_solving"], (6,9)),
    ("Magic Tree House Dinosaurs Before Dark", "Mary Pope Osborne", ["history","curiosity"], (6,10)),
    ...
    ("The Phantom Tollbooth", "Norton Juster", ["problem_solving","creativity"], (8,12)),
    ...
    ("Encyclopedia Brown Boy Detective", "Donald Sobol", ["problem_solving","curiosity"], (6,10)),
    ("From the Mixed-Up Files of Mrs. Basil E. Frankweiler", "E.L. Konigsburg", ["problem_solving","courage"], (8,12)),
    ...
    ("The Watsons Go to Birmingham 1963", "Christopher Paul Curtis", ["history","diversity","family"], (8,12)),
    ...
    ("Hidden Figures Young Readers Edition", "Margot Lee Shetterly", ["science","diversity","courage"], (8,13)),
...
async def search_open_library(title, author):
    async with httpx.AsyncClient() as client:
        r = await client.get(
            ...
            headers={"User-Agent": "BookMind/1.0 (contact@getbookmind.ai)"},
            timeout=20.0
        )
        r.raise_for_status()
...
async def run():
    ...
    """)
    print(f"📋 ID column default: {id_default}")

    # Fix id default if missing
    if not id_default:
        await conn.execute("""
            ALTER TABLE books ALTER COLUMN id SET DEFAULT gen_random_uuid()
        """)
        print("✅ Fixed id column default")

    print(f"\n📚 Adding {len(MISSING)} missing children's books")
    print("─" * 55)
    ...
    for title, author, goals, age in MISSING:
        # Check by title + author last name
        ...
        for raw in results:
            ...
            try:
                text = (
                    f"Title: {raw.get('title', title)}\n"
                    f"Author: {', '.join(raw.get('author_name') or [author])}\n"
                    f"Children's book ages {age[0]}-{age[1]}\n"
                ...
                await conn.execute("""
                    INSERT INTO books (
                        id, isbn_13, isbn_10, title, author, published_year,
                        ...
                        $1,$2,$3,$4,$5,$6,$7,$8::jsonb,$9,$10::vector,
                        'en',false,false,false,false,$11,$12,$13::jsonb,TRUE
                    ) ON CONFLICT DO NOTHING
                """,
                    new_id, isbn_13, isbn_10,
                    raw.get("title", title),
    ...
    print(f"\n{'═'*55}")
    print(f"➕ Added: {added}  ✅ Tagged: {tagged}  ❌ Failed: {len(failed)}")
    print(f"📚 Total children's books: {total}")

    print("\n📊 Books per learning goal:")
    goals_list = ["kindness","courage","friendship","emotions","science",
    ...
    if all_good:
        print("\n🎉 All learning goals have 10+ books — ready to ship!")
    else:
        print(f"\n⚠️  {len(failed)} books still missing.")
        if failed:
            for f in failed[:10]:
                print(f"   • {f}")
